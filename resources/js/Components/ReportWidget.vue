...
<template>
    <div class="fixed bottom-6 left-6 z-[100] font-sans">
            ...
            <div v-if="isOpen" class="absolute bottom-16 left-0 w-80 sm:w-96 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col mb-2">
                ...
                <div class="bg-cyan-600 p-4 flex justify-between items-center shadow-md">
                    <h3 class="text-white font-bold flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 5.636a9 9 0 11-12.728 0m12.728 0A9 9 0 015.636 18.364m12.728-12.728L5.636 18.364"></path></svg>
                        Pusat Bantuan
                    </h3>
                ...
                <div v-if="activeTab === 'form'" class="p-5 max-h-[60vh] overflow-y-auto">
                    <form @submit.prevent="submitReport" class="space-y-4">
                        ...
                        <button type="submit" :disabled="form.processing" class="w-full py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-lg text-sm transition disabled:opacity-50">
                ...
                <div v-if="activeTab === 'history'" class="p-0 max-h-[60vh] overflow-y-auto custom-scrollbar bg-slate-950">
                    ...
                    <div v-for="item in histories" :key="item.id" class="border-b border-slate-800 p-4 hover:bg-slate-900 transition">
                        <div class="flex justify-between items-start mb-2">
                            ...
                            <span :class="getStatusBadge(item.status)" class="px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wider">{{ getStatusText(item.status) }}</span>
                        </div>
        ...
        <button @click="toggleWidget" class="w-14 h-14 bg-cyan-600 hover:bg-cyan-500 text-white rounded-full shadow-[0_0_20px_rgba(8,145,178,0.5)] flex items-center justify-center transition-transform hover:scale-110 active:scale-95">
